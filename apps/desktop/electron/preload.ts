...
type ClaudeStateCallback = (sessionId: string, state: string, isClaudeActive: boolean) => void
...
const claudeStateCallbacks: Set<ClaudeStateCallback> = new Set()
...
const dropdownClaudeStateCallbacks: Set<ClaudeStateCallback> = new Set()
...
function initializeListeners() {
  ...
  ipcRenderer.on('session:claude-state', (_event: unknown, sessionId: string, state: string, isClaudeActive: boolean) => {
    claudeStateCallbacks.forEach(cb => cb(sessionId, state, isClaudeActive))
  })
  ...
  ipcRenderer.on('dropdown:claude-state', (_event: unknown, sessionId: string, state: string, isClaudeActive: boolean) => {
    dropdownClaudeStateCallbacks.forEach(cb => cb(sessionId, state, isClaudeActive))
  })
}
...
contextBridge.exposeInMainWorld('electronAPI', {
  ...
  onSessionClaudeState: (callback: ClaudeStateCallback) => {
    initializeListeners()
    claudeStateCallbacks.add(callback)
    return () => claudeStateCallbacks.delete(callback)
  },
  ...
  onDropdownClaudeState: (callback: ClaudeStateCallback) => {
    initializeListeners()
    dropdownClaudeStateCallbacks.add(callback)
    return () => dropdownClaudeStateCallbacks.delete(callback)
  },
  ...
  // Hooks
  checkHooksStatus: () => ipcRenderer.invoke('hooks:check-status'),
  setupHooks: () => ipcRenderer.invoke('hooks:setup'),
  getHooksPaths: () => ipcRenderer.invoke('hooks:get-paths'),
