...
import { claudeStateDetector } from './claudeStateDetector'
...
`)
    ...
    // Initialize Claude state detection for this session
    claudeStateDetector.initSession(sessionId, targetCwd)
    claudeStateDetector.setStateChangeCallback(sessionId, (info) => {
      if (!webContents.isDestroyed()) {
        webContents.send('session:claude-state', sessionId, info.state, info.isClaudeActive)
      }
    })
    ...
    ptyProcess.onData((data) => {
      if (!webContents.isDestroyed()) {
        ...
        if (osc7Match) {
          ...
          // Update cwd in Claude state detector for hooks matching
          claudeStateDetector.updateSessionCwd(sessionId, currentDir)
        }
        ...
        // Process Claude state detection
        claudeStateDetector.processData(sessionId, data)
  ...
  write(sessionId: string, data: string): void {
    ...
    if (session) {
      // Detect Ctrl+C and notify to clear ports and Claude state
      if (data === '\x03') {
        if (!session.webContents.isDestroyed()) {
          session.webContents.send('session:port-clear', sessionId)
        }
        // Notify Claude state detector that Ctrl+C was pressed
        claudeStateDetector.notifyCtrlC(sessionId)
      }
  ...
  destroy(sessionId: string): void {
    const session = this.sessions.get(sessionId)
    if (session) {
      claudeStateDetector.destroySession(sessionId)
      session.pty.kill()
      this.sessions.delete(sessionId)
    }
  }
...
`)
    ...
    // Initialize Claude state detection for dropdown session
    claudeStateDetector.initSession(sessionId, targetCwd)
    claudeStateDetector.setStateChangeCallback(sessionId, (info) => {
      if (this.dropdownWindow) {
        this.dropdownWindow.webContents.send('dropdown:claude-state', sessionId, info.state, info.isClaudeActive)
      }
    })
  ...
  writeDropdown(sessionId: string, data: string): void {
    ...
    if (session) {
      // Detect Ctrl+C and notify Claude state detector
      if (data === '\x03') {
        claudeStateDetector.notifyCtrlC(sessionId)
      }
      session.pty.write(data)
